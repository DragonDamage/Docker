...
$ docker run -d -p 8080:80 nginx  # Создание и запуск контейнера на основе определенного образа Docker в фоновом режиме (-d detouch)
...
$ docker exec -it {CONTAINER_ID} command   # Выполнить команду внутри запущенного контейнера (например bash, psql, etc)
